...
class MakeDataSet:
    ...
    def prepare_dataset(self):
        ...
        for patient in tqdm(self.IDRI_list):
            ...
            # for i,nod in enumerate(nodules_annotation):
            #     print("Nodule %d has %d annotations." % (i+1, len(nodules_annotation[i])))
            # print(scan)
            ...
            if len(nodules_annotation) > 0:
                ...
                # This current for loop iterates over total number of nodules in a single patient
                    ...
                    total_slice += (cbbox[2].stop - cbbox[2].start)
                ...
                # print(nodule_min_start, nodule_max_stop)        
                patient_slice_range = np.append(patient_slice_range, np.array([[nodule_min_start, nodule_max_stop]]), axis=0)
                # print('patient_slice_range', patient_slice_range.shape, patient_slice_range, total_slice )
    ...
    def prepare_dataset_clean(self, patient_slice_range, total_slice):
            ...
            # This is to name each image and mask
            prefix = [str(x).zfill(3) for x in range(1000)]
            per_nodule_slice = total_slice/250
            ...
            for patient in tqdm(self.IDRI_list):
                ...
                if len(nodules_annotation) > 0:
                    ...
                    # print(stop_1, start_2, patient_slice_range[positive_patient_count])
                    # 分前後兩部分
                    ...
                    if start_2 + per_nodule_slice/2 <= vol.shape[2]:
                        ...
                        stop_2 = vol.shape[2]
                    # print(start_1, stop_1, ',', start_2, stop_2)    
                    # 前半部分
                    for slice_id in range(int(start_1), int(stop_1)):
                        ...
                        #CN= CleanNodule, CM = CleanMask
                        nodule_name = "{}_CN001_slice{}".format(pid[-4:],prefix[slice_id])
                        mask_name = "{}_CM001_slice{}".format(pid[-4:],prefix[slice_id])
                        meta_list = [pid[-4:],slice_id,prefix[slice_id],nodule_name,mask_name,0,False,True]
                        self.save_meta(meta_list)
                    ...
                    slice_count = 0
                    for slice_vol in vol[clean_slice]:
                        # if slice >= per_nodule_slice:
                        #     break
                        ...
                        #CN= CleanNodule, CM = CleanMask
                        nodule_name = "{}_CN001_slice{}".format(pid[-4:],prefix[slice_count])
                        mask_name = "{}_CM001_slice{}".format(pid[-4:],prefix[slice_count])
                        meta_list = [pid[-4:],slice_count,prefix[slice_count],nodule_name,mask_name,0,False,True]
                        self.save_meta(meta_list)
...
if __name__ == '__main__':
    ...
    print(total_slice, patient_slice_range.shape)
